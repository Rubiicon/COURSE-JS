...
const getUsersWithFriend = (users, friendName) => {
  return users.filter(user => user.friends.find(friend => friend === friendName)).map(friendName => friendName.name);
};
...
const getNamesSortedByFriendsCount = users => {
  return users.sort((next, prev) => next.friends.length - prev.friends.length).map(user => user.name);
};
